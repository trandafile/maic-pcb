...
REM Assicura esclusioni critiche (No secrets)
if not exist ".gitignore" (
    (echo # Gitignore generated by MAIC setup
     echo hipa/
     echo credenziali-bot.json
    ...
    if errorlevel 1 echo hipa/>> .gitignore
    findstr /R /X /C:"credenziali-bot.json" .gitignore >nul 2>&1
    if errorlevel 1 echo credenziali-bot.json>> .gitignore
    findstr /R /X /C:".streamlit/secrets.toml" .gitignore >nul 2>&1
    if errorlevel 1 echo .streamlit/secrets.toml>> .gitignore
)
...
if "%GITHUB_TOKEN%"=="" (
    ...
    set "PUSH_URL=%REPO_URL:https://=https://%GITHUB_TOKEN%@%"
    git push -u "!PUSH_URL!" %BRANCH% > "%PUSH_LOG%" 2>&1
)
type "%PUSH_LOG%"
if errorlevel 1 goto ERRORE
